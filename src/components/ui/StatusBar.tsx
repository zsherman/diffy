...
function CodeRabbitIcon({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 32 32" width="14" height="14" fill="currentColor">
...
function CLIIndicator({ 
  ...
  return (
    <div
      ...
      title={available ? `${name} is installed` : `${name} not found. ${installInstructions}`}
    >
      <span
      ...
      />
      <Icon className={`w-3 h-3 ${available ? 'text-text-muted' : 'text-text-muted opacity-30'}`} />
      <span className={available ? 'text-text-muted' : 'text-text-muted opacity-30'}>
        {name}
      </span>
    </div>
  );
