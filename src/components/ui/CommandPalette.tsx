...
import { InteractiveRebaseDialog } from "../../features/commits/components/InteractiveRebaseDialog";
...
export function CommandPalette() {
  ...
  >([]);
  const [branchesForInteractiveRebase, setBranchesForInteractiveRebase] =
    useState<Array<{ name: string; isHead: boolean; isRemote: boolean }>>([]);
  const [selectedInteractiveRebaseRef, setSelectedInteractiveRebaseRef] =
  ...
  const handleOpenInteractiveRebasePage = async () => {
    if (!repository) return;
    try {
      const branches = await listBranches(repository.path);
      // Include both local and remote branches for rebase
      setBranchesForInteractiveRebase(
        branches.map((b) => ({
          name: b.name,
          isHead: b.isHead,
          isRemote: b.isRemote,
      ...
      setSearch("");
      setPages([...pages, "interactive-rebase"]);
    } catch (error) {
      toast.error("Failed to load branches", getErrorMessage(error));
    }
  };

  const handleSelectInteractiveRebaseRef = (ontoRef: string) => {
    setSelectedInteractiveRebaseRef(ontoRef);
    setShowCommandPalette(false);
  };
  ...
  return (
    <>
      ...
      >
        ...
        <div className="relative w-full max-w-2xl bg-bg-secondary rounded-lg shadow-xl border border-border-primary overflow-hidden">
          ...
          {page && (
            <div className="flex items-center gap-2 px-4 py-2 border-b border-border-primary bg-bg-tertiary">
              ...
              <span className="text-sm text-text-muted">
                ...
                {page === "interactive-rebase" && "Interactive Rebase Onto..."}
          ...
          <Command.List className="max-h-80 overflow-y-auto p-2">
            ...
            {/* Interactive Rebase Page */}
            {page === "interactive-rebase" && (
              <>
                {branchesForInteractiveRebase
                  .filter((b) => !b.isHead) // Don't show current branch
                  .map((branch) => (
                    <Command.Item
                      key={branch.name}
                      onSelect={() =>
                        handleSelectInteractiveRebaseRef(branch.name)
                      }
                      className="flex items-center gap-3 px-2 py-2 rounded-sm cursor-pointer text-text-primary data-[selected=true]:bg-bg-hover text-sm"
                    >
                      <GitFork
                ...
                {branchesForInteractiveRebase.filter((b) => !b.isHead)
                  .length === 0 && (
                  <div className="py-6 text-center text-text-muted text-sm">
                    No other branches available for interactive rebase
                  </div>
                )}
              </>
            )}
            ...
            {page === "themes" && (
              <>
                {THEMES.map((t) => (
                  <Command.Item
                    key={t.id}
                    onSelect={() => runCommand(() => setTheme(t.id as ThemeId))}
                    className="flex items-center gap-3 px-2 py-2 rounded-sm cursor-pointer text-text-primary data-[selected=true]:bg-bg-hover text-sm"
                  >
            ...
            {!page && (
              <>
                ...
                >
                  ...
                  <Command.Item
                    onSelect={() =>
                      runCommand(() => setShowFileTreePanel(!showFileTreePanel))
                    }
                    className="flex items-center gap-3 px-2 py-2 rounded-sm cursor-pointer text-text-primary data-[selected=true]:bg-bg-hover text-sm"
                  >
                    <TreeStructure size={16} className="text-text-muted" />
                    <span className="flex-1">Toggle File Tree Panel</span>
                    <span className="text-xs text-text-muted">
                      {showFileTreePanel ? "Hide" : "Show"}
                    </span>
                  </Command.Item>
                ...
                {repository && (
                  ...
                  >
                    ...
                    >
                      ...
                      <span className="text-xs text-text-muted">
                        {appView === "workspace" && mainView === "repository"
                          ? "Active"
                          : ""}
                      </span>
                    </Command.Item>
                    ...
                    >
                      ...
                      <span className="text-xs text-text-muted">
                        {appView === "workspace" && mainView === "statistics"
                          ? "Active"
                          : ""}
                      </span>
                    </Command.Item>
                    ...
                    >
                      ...
                      <span className="text-xs text-text-muted">
                        {appView === "workspace" && mainView === "changelog"
                          ? "Active"
                          : ""}
                      </span>
                    </Command.Item>
                ...
                {repository && (
                  ...
                  >
                    ...
                    <Command.Item
                      onSelect={() => handleOpenInteractiveRebasePage()}
                      className="flex items-center gap-3 px-2 py-2 rounded-sm cursor-pointer text-text-primary data-[selected=true]:bg-bg-hover text-sm"
                      keywords={[
                        "interactive",
                        "rebase",
                    ...
                    >
                      <GitFork size={16} className="text-accent-purple" />
                      <span className="flex-1">Interactive Rebase...</span>
                      <CaretRight size={16} className="text-text-muted" />
                    </Command.Item>
                  </Command.Group>
                )}
                ...
                >
                  <Command.Item
                    onSelect={() =>
                      runCommand(() =>
                        setPerfTracingEnabled(!perfTracingEnabled),
                      )
                    }
                    className="flex items-center gap-3 px-2 py-2 rounded-sm cursor-pointer text-text-primary data-[selected=true]:bg-bg-hover text-sm"
                ...
                >
                  <Command.Item
                    onSelect={() =>
                      runCommand(() => setShowSettingsDialog(true))
                    }
                    className="flex items-center gap-3 px-2 py-2 rounded-sm cursor-pointer text-text-primary data-[selected=true]:bg-bg-hover text-sm"
                  >
      ...
      {selectedInteractiveRebaseRef && repository && (
        <InteractiveRebaseDialog
          repoPath={repository.path}
          ontoRef={selectedInteractiveRebaseRef}
          onClose={() => setSelectedInteractiveRebaseRef(null)}
          onSuccess={() => setSelectedInteractiveRebaseRef(null)}
        />
      )}
    </>
  );
